...
def test_partial_session_is_listed_opened_and_recovered(tmp_path):
    ...
    assert summaries[0].is_partial is True
    assert summaries[0].frames_received == 20

    opened_partial = repository.load_window(
        summaries[0].session_id,
        start_us=1_000_000,
        end_us=2_000_000,
        max_points=100,
    )
    assert opened_partial.loaded_frames == 20

    result = repository.recover_partial(summaries[0].session_id)
    assert result.frames_recovered == 20
    assert result.frames_discarded == 0
    assert result.output_path.exists()
    assert not partial_path.exists()

    recovered = repository.list_sessions()[0]
    assert recovered.is_partial is False
    assert recovered.state == "recovered_partial"
    assert recovered.integrity_status == "verified"
    assert repository.verify_integrity(recovered.session_id) == "verified"
