...
def test_parse_valid_frame_assigns_temporal_fields_and_channels():
    ...
    parsed = FrameCsvParser().parse_line(
        "FRAME,10,20,1000000,0.52,0.81,97",
        session,
    )

    assert parsed.frame.packet_sequence == 10
    assert parsed.frame.scan_sequence == 20
...
@pytest.mark.parametrize(
    ...
    [
        ("FRAME,-1,0,0,1,2,3", "packet_sequence fora da faixa"),
        (f"FRAME,{UINT32_MAX + 1},0,0,1,2,3", "packet_sequence fora da faixa"),
        (f"FRAME,0,0,{UINT64_MAX + 1},1,2,3", "timestamp_us fora da faixa"),
        ("FRAME,0,0,0,nan,2,3", "NaN ou infinitos"),
        ("FRAME,0,0,0,inf,2,3", "NaN ou infinitos"),
    ],
)
...
def test_format_frame_csv_uses_stage12_contract():
    line = format_frame_csv(5, 8, 1_234_567, [1.0, 2.5])
    assert line == "FRAME,5,8,1234567,1.0,2.5"
